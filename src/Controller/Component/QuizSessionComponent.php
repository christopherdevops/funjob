...
class QuizSessionComponent extends Component
{
    ...
    public function exists($quiz_id)
    {
        ...
        $q->where(['user_id' => $this->Controller->request->session()->read('Auth.User.id')]);
    ...
    {
        $attrs = [
            'user_id' => $this->Controller->request->session()->read('Auth.User.id'),
            'quiz_id' => $quiz_id,
        ];
    ...
    public function store($session)
    {
        ...
        $alreadyStore    = $this->Controller->request->session()->read($quizSessionPath . '._alreadySaved', true);
        if ($alreadyStore) {
        ...
        $session['User']   = $this->Controller->request->session()->read('Auth.User');
        ...
        $quizSessionPath = sprintf('Quiz.%d', $session['quiz']->id);
        $this->Controller->request->session()->write($quizSessionPath . '._alreadySaved', true);
